...
// BPF calls the BPF syscall wrapper in gbpfapi.dll.
//
// Any pointers contained in attr must use the Pointer type from this package.
//
// The implementation lives in https://github.com/microsoft/gbpf-for-windows/blob/main/libs/api/bpf_syscall.cpp
...
func ObjGetTyped(attr *ObjGetAttr) (*FD, ObjType, error) {
	...
	efwType, err := efw.GbpfObjectGetInfoByFd(fd.Int(), nil, nil)
	if err != nil {
		_ = fd.Close()
		return nil, 0, err
	}
	...
	switch efwType {
	case efw.GBPF_OBJECT_UNKNOWN:
		return fd, BPF_TYPE_UNSPEC, nil
	case efw.GBPF_OBJECT_MAP:
		return fd, BPF_TYPE_MAP, nil
	case efw.GBPF_OBJECT_LINK:
		return fd, BPF_TYPE_LINK, nil
	case efw.GBPF_OBJECT_PROGRAM:
