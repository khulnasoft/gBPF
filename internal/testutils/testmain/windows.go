...
//
// See https://github.com/microsoft/gbpf-for-windows/blob/main/libs/shared/gbpf_tracelog.h
...
// newTraceSession starts a trace log for gBPF for Windows related events.
//
// * https://github.com/microsoft/gbpf-for-windows/blob/main/docs/GettingStarted.md#using-tracing
...
func newTraceSession() (*traceSession, error) {
	def := filepath.Join(os.Getenv("ProgramFiles"), "gbpf-for-windows\\gbpfforwindows.wprp")
...
func (ts *traceSession) Dump(w io.Writer) error {
	...
	path, err := os.MkdirTemp("", "gbpf-go-trace")
	if err != nil {
		return err
	}
...
func summariseWPRTrace(r io.Reader, w io.Writer) error {
	...
	for _, event := range events.Events {
		if !strings.Contains(event.System.Provider.Name, "Gbpf") {
			continue
		}
