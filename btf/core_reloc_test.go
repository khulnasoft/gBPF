...
func TestCORERelocationLoad(t *testing.T) {
	...
	spec, err := gbpf.LoadCollectionSpecFromReader(fh)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, progSpec := range spec.Programs {
		t.Run(progSpec.Name, func(t *testing.T) {
			...
			prog, err := gbpf.NewProgramWithOptions(progSpec, gbpf.ProgramOptions{
...
func TestLD64IMMReloc(t *testing.T) {
	...
	coll, err := gbpf.NewCollection(spec)
...
func TestCOREPoisonLineInfo(t *testing.T) {
	spec, err := gbpf.LoadCollectionSpec(testutils.NativeFile(t, "../testdata/errors-%s.elf"))
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			_, err := gbpf.NewProgramWithOptions(progSpec, gbpf.ProgramOptions{
				KernelTypes: empty,
			})
			testutils.SkipIfNotSupported(t, err)

			var ve *gbpf.VerifierError
