...
func loadCollectionFromNativeImage(file string) (_ *Collection, err error) {
	...
	nMaps, nPrograms, err := efw.GbpfObjectLoadNativeFds(file, mapFds, programFds)
	if errors.Is(err, efw.GBPF_NO_MEMORY) && (nMaps > len(mapFds) || nPrograms > len(programFds)) {
	...
	for _, raw := range mapFds {
		...
		_, err = efw.GbpfObjectGetInfoByFd(m.FD(), unsafe.Pointer(&efwMapInfo), &size)
		if err != nil {
			_ = m.Close()
			return nil, err
		}
	...
	for _, raw := range programFds {
		...
		_, err = efw.GbpfObjectGetInfoByFd(program.FD(), unsafe.Pointer(&efwProgInfo), &size)
		if err != nil {
			_ = program.Close()
			return nil, err
		}
