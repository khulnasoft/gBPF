...
func TestUprobeMulti(t *testing.T) {
	testutils.SkipIfNotSupported(t, havgBPFLinkUprobeMulti())

	prog := mustLoadProgram(t, gbpf.Kprobe, gbpf.AttachTraceUprobeMulti, "")
...
func TestUprobeMultiProgramCall(t *testing.T) {
	...
	test := func(retprobe bool, expected uint32) {
		m, p := newUpdaterMapProg(t, gbpf.Kprobe, gbpf.AttachTraceUprobeMulti)
...
func TestHavgBPFLinkUprobeMulti(t *testing.T) {
	testutils.CheckFeatureTest(t, havgBPFLinkUprobeMulti)
}
