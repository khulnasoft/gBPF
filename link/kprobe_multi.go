...
type KprobeMultiOptions struct {
	...
	// [gbpf.AttachTraceKprobeSession] attach type.
	//
...
func kprobeMulti(prog *gbpf.Program, opts KprobeMultiOptions, flags uint32) (Link, error) {
	...
	if opts.Session {
		if haveFeatErr := havgBPFLinkKprobeSession(); haveFeatErr != nil {
	...
	if errors.Is(err, unix.EINVAL) {
		return nil, fmt.Errorf("%w (missing kernel symbol or prog's AttachType not %s?)", err, gbpf.AttachType(attachType))
	}
...
func (kml *kprobeMultiLink) Update(_ *gbpf.Program) error {
...
var havgBPFLinkKprobeMulti = internal.NewFeatureTest("bpf_link_kprobe_multi", func() error {
...
var havgBPFLinkKprobeSession = internal.NewFeatureTest("bpf_link_kprobe_session", func() error {
	prog, err := gbpf.NewProgram(&gbpf.ProgramSpec{
		Name: "probe_kps_link",
		Type: gbpf.Kprobe,
		Instructions: asm.Instructions{
		...
		AttachType: gbpf.AttachTraceKprobeSession,
		License:    "MIT",
	})
	if errors.Is(err, unix.E2BIG) {
		// Kernel doesn't support AttachType field.
