...
func (ex *Executable) UprobeMulti(symbols []string, prog *gbpf.Program, opts *UprobeMultiOptions) (Link, error) {
	return ex.uprobeMulti(symbols, prog, opts, 0)
}

func (ex *Executable) UretprobeMulti(symbols []string, prog *gbpf.Program, opts *UprobeMultiOptions) (Link, error) {
...
func (ex *Executable) uprobeMulti(symbols []string, prog *gbpf.Program, opts *UprobeMultiOptions, flags uint32) (Link, error) {
	...
	if err != nil {
		if haveFeatErr := havgBPFLinkUprobeMulti(); haveFeatErr != nil {
...
var havgBPFLinkUprobeMulti = internal.NewFeatureTest("bpf_link_uprobe_multi", func() error {
	prog, err := gbpf.NewProgram(&gbpf.ProgramSpec{
		...
		AttachType: gbpf.AttachTraceUprobeMulti,
		License:    "MIT",
	})
