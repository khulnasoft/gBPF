...
func CollectGlobalTypes(spec *gbpf.CollectionSpec) []btf.Type {
	var types []btf.Type
...
func collectMapTypes(types []btf.Type, maps map[string]*gbpf.MapSpec) []btf.Type {
	for _, m := range maps {
...
func collectVariableTypes(types []btf.Type, vars map[string]*gbpf.VariableSpec) []btf.Type {
	for _, vs := range vars {
		v := vs.Type()
		if v == nil {
			continue
